# 💰 SAMART — Pricing & Revenue Model

---

## Pricing Tiers

### 🌱 Starter — 1,490 บาท/เดือน
**"เริ่มต้นใช้ AI ไม่ต้องกลัว"**

เหมาะสำหรับ: ร้านอาหาร/คาเฟ่ที่เพิ่งเริ่ม digitalize

| Feature | รายละเอียด |
|---------|------------|
| SAMART **Talk** (Chat Brain) | LINE OA chatbot พื้นฐาน รับออเดอร์ + ตอบ FAQ |
| SAMART **Guide** (Insight) | Dashboard ยอดขายรายวัน + weekly AI summary |
| LINE OA Connection | 1 บัญชี |
| Products/Menu | สูงสุด 50 รายการ |
| AI messages | 1,000 ข้อความ/เดือน |
| Support | Email support |

---

### 🚀 Pro — 2,490 บาท/เดือน ⭐ แนะนำ
**"AI ทำงานแทนทีมเต็มรูปแบบ"**

เหมาะสำหรับ: ร้านที่ต้องการ marketing และ data ครบวงจร

รวมทุกอย่างใน Starter บวก:

| Feature | รายละเอียด |
|---------|------------|
| SAMART **Create** (Content Brain) | สร้างโพสต์ Facebook/LINE อัตโนมัติ 30 โพสต์/เดือน |
| SAMART **Create** — Image AI | สร้างรูปโฆษณาด้วย Gemini Imagen |
| SAMART **Sense** (Sales Brain) | Sales forecast 30 วัน + stock alert |
| Customer Memory | จำประวัติลูกค้า + follow-up อัตโนมัติ |
| AI messages | ไม่จำกัด |
| Products/Menu | ไม่จำกัด |
| Support | Chat support (response < 4 ชม.) |

---

### 💎 Business — 3,490 บาท/เดือน
**"สำหรับเจ้าของที่มีหลายสาขา"**

รวมทุกอย่างใน Pro บวก:

| Feature | รายละเอียด |
|---------|------------|
| Multi-branch | สูงสุด 5 สาขา |
| Staff Accounts | สูงสุด 10 accounts |
| API Access | Connect กับระบบภายนอก |
| Priority Support | Response < 1 ชม. + Line Direct |
| Custom AI Training | Train AI จากข้อมูลร้านคุณโดยเฉพาะ |
| Advanced Analytics | Cohort analysis + LTV prediction |

---

## Add-ons (ซื้อเพิ่มได้)

| Add-on | ราคา |
|--------|------|
| Video Content AI (Reels/TikTok) | +500 บาท/เดือน |
| Extra LINE OA | +300 บาท/บัญชี/เดือน |
| Extra Branch | +800 บาท/สาขา/เดือน |
| WhatsApp Integration | +400 บาท/เดือน |
| Setup & Onboarding | 1,990 บาท (ครั้งเดียว) |

---

## Revenue Projection

### Year 1 Monthly Recurring Revenue (MRR)

| เดือน | ลูกค้า Starter | ลูกค้า Pro | ลูกค้า Business | MRR รวม |
|-------|---------------|------------|-----------------|---------|
| 1 | 5 (pilot ฟรี) | 0 | 0 | 0 |
| 2 | 8 (pilot ฟรี) | 2 | 0 | 4,980 |
| 3 | 10 | 5 | 0 | 27,400 |
| 4 | 15 | 15 | 2 | 66,730 |
| 5 | 18 | 20 | 3 | 114,190 |
| 6 | 20 | 25 | 5 | 114,300 |
| 9 | 25 | 40 | 8 | 156,570 |
| 12 | 30 | 55 | 15 | 232,500 |

> **Year 1 ARR เป้าหมาย**: ~2,500,000 บาท (ณ สิ้นปี)

---

## Unit Economics

| Metric | ค่า |
|--------|-----|
| ARPU เฉลี่ย | ~2,100 บาท/เดือน |
| Gross Margin | ~80% (SaaS) |
| CAC ประมาณ | ~3,000 บาท/ลูกค้า |
| LTV ประมาณ (12 เดือน) | ~25,200 บาท |
| LTV:CAC Ratio | 8.4x ✅ |
| Churn target | < 5%/เดือน |

---

## Pricing Philosophy

1. **ราคาต้องถูกกว่าเงินเดือนพนักงาน 1 คน** — พนักงาน part-time เชียงใหม่ได้ ~10,000-12,000 บาท/เดือน SAMART Pro แค่ 2,490 บาท
2. **ราคาต้องน้อยกว่าค่าคอมมิชชัน Grab** — ถ้าร้านขายผ่าน Grab 50,000 บาท/เดือน โดนหัก 30% = 15,000 บาท SAMART ช่วยให้ลูกค้ามาสั่งตรง ประหยัดได้มากกว่า 10 เท่า
3. **Annual discount 20%** — จูงใจให้จ่ายรายปี ลด churn
